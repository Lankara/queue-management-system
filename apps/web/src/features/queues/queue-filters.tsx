...
export function QueueFilters({
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <Select label="Service filter" placeholder="All services" value={serviceId} onChange={(event) => onServiceChange(event.target.value)} options={services.map((service) => ({ label: service.name, value: service.id }))} />
    </div>
  );
}
